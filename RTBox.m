...
% power (v4.2 or earlier) from USB port. However, you can do this anytime.
% The optional second input specifies how long the test will last (default
% 30 s). If you want to return computer time, it is better to do this test
% once before experiment. The program will automatically use the test
% result to correct device time. For version 1.1 or earlier, you need the
% write privilege to save the ratio to a file, so the code can retrieve it
% next time. If you don't have this privilege, you have to correct the
% ratio each time after you close the device.
% 
...
% returned events will be string array. If there are more than one events,
% it will be a cell string.
% 
...
% are detected. If there are events available in the buffer, this function
% will read back all of them. To set the number of events to wait, use
% RTBox('nEventsRead', n).
%
% [oldValue=] RTBox('nEventsRead', nEvents); 
% - Set the number of events (default 1) to wait for during read functions.
% For RTBox('trigger'), this refers to the number of events besides the
% trigger. If you want the read functions to wait for more events, set
% nEvents accordingly. Limitation to v1.4 or earlier: the repeated events 
% due to button bouncing are counted as different events.
% 
...
% which normally indicates the computer is receiving data.
% 
% [timing, events] = RTBox('BoxSecs', timeout);
...
% as 'sound'. The trigger can also be any enabled event. Since the timing
% is relative to the provided trigger, both the trigger timing, which is 0,
% and the trigger event are omitted in output. Normally the trigger
% indicates the onset of stimulus, so the returned time will be response
% time. By default, this will wait for one event besides the trigger event.
% The n in RTBox('nEventsRead', n) doesn't include the trigger event. If
% the trigger event is not detected, there will be a warning and both
% output will be empty, no matter whether there are other events.
% 
% [oldValue=] RTBox('DebounceInterval', intervalSecs); 
% - Set/get debounce interval in seconds. RTBox will ignore both button
% down and up events within intervalSecs window after an event of the same
% button. The valid intervalSecs is from 0 to about 0.283s (v4.3 or
% earlier), default 0.05. intervalSecs=0 will disable debouncing. The
% debouncing is performed in Matlab code for v1.4 and earlier, and in
% device firmware for later versions. Note that our debounce schemes won't
% cause any time delay for button response.
% 
% Note that prior to v4.4, RTBox always use default 0.05s when the device
% is opened. Since v4.4, the setting is saved even after the power is lost.
% 
...
% 'light' 'TR' or 'aux', or cellstr containing any of these strings. The
% string 'all' is a shortcut for all the events. By default, only 'press'
% is enabled. If you want to detect button release time instead of button
...
% - Send a trigger to RTBox via the serial port. This is obsolete and may
% be removed in the future. You are not suggested to use this, since the
% built-in clock sync always gives you better accuracy. The optional output
% are computer time when the trigger was sent, and its upper bound.
% 
% [timeSent, timeSentUb =] RTBox('TTL', eventCode);
% - Send TTL to DB-25 port (pin 8 is bit 0). The second input is event code
% (default 1), 4-bit (0~15) for v<5, and 8-bit (0~255) for v>=5. It can
% also be equivalent binary string, such as '0011'. The optional output are
% the time the TTL was sent, and its upper bound. The width of TTL is
% controlled by RTBox('TTLWidth') command. TTL function is supported only
% for v3.0 and later, which was designed for EEG event code.
% 
% [oldValue =] RTBox('TTLWidth', widthSecs);
% - Set/get TTL width in seconds. The default width is 0.97e-3. The actual
% width may have some small variation. The supported width by the hardware
% ranges from 0.14e-3 to 35e-3 secs. The infinite width is also supported.
% Infinite width means the TTL will stay until it is changed by next
% RTBox('TTL') command, such as RTBox('TTL', 0).
% 
% If width longer than 35ms is needed, the solution is to use infinite
% width, and turn off the TTL after widthSecs like this:
% 
...
% 
% If your code can't afford to wait after sending TTL, timer can be used to
% turn off the TTL. You need to create a single shot timer and set infinite
% width early in your code. The TimerFcn is to turn off the TTL:
...
% 
% Note that prior to v4.4, RTBox always use default width (~1ms) when the
% device is opened. Since v4.4, the setting is saved even after power off.
% 
...
% know.
% 
% Note that prior to v4.4, RTBox always use default level (0) when the
% device is opened. Since v4.4, the setting is saved even after the power
% is lost.
% 
% In v>=5, newLevel has second value, which is the polarity for pins 17~24.
% 
% [timing =] RTBox('WaitTR');
% - Wait for TR, and optionally return accurate TR time based on computer
% clock. This command will enable TR detection automatically, so you do not
% need to do it in your code. You can press key 5 to simulate TR for test.
% 
% [bufferSz =] RTBox('BufferSize' [, newSize]);
% - Set/get input buffer size in number of events. The default buffer can
% hold about 585 events, which is enough for most experiments. If you need
% to buffer more events and read all once after long period of time, you
% can set a new larger newSize.
% 
% [oldThre =] RTBox('threshold' [, newThre]); 
% - Set/get threshold for sound and light trigger. There are four levels
% (1:4) of the threshold. Default (1) is the lowest. If, for example, the
% background light is relatively high so the device detects light trigger
% at background, you can increase the threshold to a higher level. Note
% that the device will save the setting till you change it next time. This
% function is only available for v5+ hardware. The threshold controls the
% comparator reference voltage. In early versions, it is controlled by a
% potentiometer inside the box, and one has to open the box to adjust it.
% 
% RTBox('test');
...
% you can use keyboard to respond. The time will be from KbCheck. To allow
% your code works under fake mode, the button names must be supported key
% names returned by RTBox('KeyNames'). Some commands will be ignored
% silently at fake mode. It is recommended you set fake to 1 in the Command
% Window before you test your code. Then you can simply clear all to exit
% fake mode. If you want to use keyboard for experiment, you can insert
% RTBox('fake', 1) in your code before any RTBox call. Then if you want to
% switch to response box, remember to change 1 to 0.
% 
% keys = RTBox('KeyNames');
...
% 09/2009, impliment method to set latencyTimer 
% 09/2009, impliment portname to open 
...
% 12/2009, firmware update implemented in RTBoxFirmwareUpdate 
...
% 05/2011, make 'sound' subcommand equivalent to 'pulse' 
...
% 10/2011, remove dependence on new regexptranslate for Windows LatencyTimer  
% 10/2011, remove dependence on robustfit or regress (according to Craig Arnold) 
% 12/2011, minor changes: invert clock ratio byte order
% 01/2012, minor changes: make onCleanup independent of matlab version
...
% 08/2012, warn not to use RTBox('start')
...
switch cmd
    ...
    case 'ttl' % send TTL
        ...
        twin = tpost-tpre;
        tpre = tpre+tpreOffset;
        if info(id).version>=5, tpre = tpre+tpreOffset; end % 2 bytes for v5+
    ...
    case 'start' % send serial trigger to device
        ...
        if nargout, varargout={tpre twin}; end
        if twin>0.003, RTBoxWarn('USBoverload',twin); end
        fprintf(2, '\n Warning: RTBox(''start'') is not recommended to use.\n');
    case 'clear'
        ...
        while 1
            if IOPort('BytesAvailable',s)>=7
                if nargout
                    ...
                    t = bytes2secs(b7(2:7)');
                    syncClocks([], 10); % new sync
                    t = t+info(id).sync(1);
                end
                break;
            end
            [key, t]=ReadKey({'5' 'esc'}); % check key press
            if any(strcmp('5',key)), break
    ...
    case read % 13 trigger events, plus 'secs' 'boxsecs'
        ...
        if info(id).version<1.5 && info(id).debounceInterval>0 && ...
                length(unique(eventInd))<length(eventInd)
            rmvInd=[];
...
%         if ~any(info(id).enabled), RTBoxWarn('allDisabled',info(id).ID); end
    ...
    case 'clockratio' % measure clock ratio computer/box
        ...
        if info(id).version>4.2 % store ratio in EEPROM
            if ntrial>=30
                b8=typecast(info(id).clkRatio,'uint8');
                writeEEPROM(info(id).MAC(1), [b8 info(id).MAC(2:7)]);
            end
        elseif info(id).version>1.1 % store ratio in the device
    ...
    case 'ttlwidth'
        ...
        if info(id).version >= 4.4
            writeEEPROM(224, uint8(255-width/wUnit));
        else
            b8=get8bytes;
            b8(1)=width/wUnit;
    ...
    case 'reset'
        ...
        if info(id).version<4.4, b8=get8bytes; end % to restore later
        ...
        if info(id).version<4.4, set8bytes(b8); end % restore param
        syncClocks(1:2,10);
    ...
    case 'threshold'
        ...
        if info(id).version<5, RTBoxWarn('notSupported',in1,5); return; end
        if isempty(in2), in2 = 1; end
        in2 = round(in2);
        in2 = max(min(in2, 4), 1);
    ...
    case 'info'
        ...
        if nIn<2
            ...
            fprintf(' Matlab: %s\n',version);
            fprintf(' RTBox.m last updated on 04/23/2013\n');
    ...
    function b = readEEPROM(addr, nBytes)
        IOPort('Write', s, uint8(17));
        IOPort('Write', s, uint8([addr nBytes]));
        b = IOPort('Read', s, 1, nBytes);        
    end

    function writeEEPROM(addr, bytes)
        nBytes = length(bytes);
    ...
    function openRTBox
        ...
        if exist('onCleanup', 'class') % clean up automatically
            info(id).cleanObj = onCleanup(@()closeRTBox(s,v,port));
        end

        if v>4.2 % ratio in EEPROM
            if v > 4.3 % all parameters in EEPROM
                b = readEEPROM(224, 6);
            ...
            info(id).MAC(2:7) = MACAddress;
            for ic=0:15 % arbituary # of computers for a box
                b14 = readEEPROM(ic*14, 14);
                if all(b14(1:6)==255), break; end % EEPROM not written
                if all(info(id).MAC(2:7)==b14(9:14)), break; end % found it
            end
            if ic==15, ic = 0; end % all slots written
            info(id).MAC(1) = uint8(ic*14);
...
switch err
    ...
    case 'invalidPort'
        ...
        if ~isempty(p.avail) % available
            ...
            'Is it the RT device? If yes, try again. It may help to unplug ' ...
            'then plug the device. '],cell2str(p.avail));
        end
        if ~isempty(p.busy) % is busy
            str=sprintf(['%s Port %s is not available, probably used by other program. ' ...
            'Is it the RT device? If yes, try ''clear all'' to close the port.'], ...
            str, cell2str(p.busy));
        end
        if ~isempty(info) % have opened RTBox
    ...
    case 'invalidTTL'
        str=sprintf(['TTL output must be integer from 0 to %g, or ' ...
    ...
    case 'invalidInfoStuct'
        str=sprintf(['The input for info must be a struct with the same ' ...
            'fields as the one returned by RTBox(''info'').']);
...
switch err
    ...
    case 'invalidTTLwidth'
        str=sprintf('Supported TTL width is from %.2fe-3 to %.2fe-3 s .', ...
            [1 255]*varargin{1}*1000);
    case 'widthOffset'
    ...
    case 'LatencyTimerFail'
        str=sprintf(['%s This simply means failure to speed up USB-serial ' ...
            'port reading. It won''t affect RTBox function.'],varargin{1});
...
% Return MAC address in decimal
function mac = MACAddress
    mac = zeros(1, 6, 'uint8'); 
    if IsWin
        try
            nw = java.net.InetAddress.getLocalHost;
            nw = java.net.NetworkInterface.getByInetAddress(nw);
            if ~isempty(nw)
                mac = typecast(nw(1).getHardwareAddress', 'uint8');
                if ~isempty(mac), return; end
            end
        end
        % getmac is slow, so we try java approach first
        [err, str] = system('getmac /NH');
        if err, fprintf(2, str); return; end
        ind = regexp(str, '(\w+-){5}', 'once'); % xx-xx-xx-xx-xx-
        str = str(ind+(0:16)); % 3*6-1 char
    elseif IsOSX || IsLinux
        str = Screen('computer'); 
        str = str.MACAddress;
    else
        fprintf(2, 'Unsupported system.\n');
        return;
    end
    mac = zeros(1, 6, 'uint8'); 
    mac(:) = sscanf(str, '%2X%*c', 6);
end
    ...
    switch cmd
        ...
        case 'test'
            ...
            while 1
                ...
                if isempty(event), WaitSecs('YieldSecs',0.005); 
                elseif strcmp(event,'esc'), ListenChar; break;
                else
                    fprintf('%9s%12.4f\n',event,t-t0);
                    KbReleaseWait;
                end
...
% Query/change FDTI USB-serial port latency timer. The query can be done
% with restricted users, but the change requires administrator privilege.
...
elseif IsLinux
    ...
    msecs=uint8(msecs);
    if oldVal~=msecs
        % system(sprintf('setserial %s low_latency', port)); % from PsychRTBox.m
        % change it. works only for su. But it seems linux needs su anyway
